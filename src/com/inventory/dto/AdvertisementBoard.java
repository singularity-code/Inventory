package com.inventory.dto;

public class AdvertisementBoard extends KoiMaterial {

	public AdvertisementBoard() {
		super();
	}

	public AdvertisementBoard(String index, String sn, String id, String name, String type, String brand, String user,
			String previous, String campus, String location, String updatedate, String status, String comment) {
		super(index, sn, id, name, type, brand, user, previous, campus, location, updatedate, status, comment);
		// TODO Auto-generated constructor stub
	}


}
